...
public class NaiveBayes {
     ...
     * @return
     */
    private List preprocessDataset(Map<String, String[]> trainingDataset) {
     ...
     */
    public int size = 0;
    public HashMap<String, Map<String, Double>> val = new HashMap();
    public TreeMap<String, Double> rankingMap = new TreeMap<String, Double>();

    public String predict(String text, boolean disable) throws IllegalArgumentException {
...
//                System.out.println(knowledgeBase.logLikelihoods.get(feature));
//                System.out.println(knowledgeBase.logLikelihoods.get(feature).get(category));

//                val.put(feature, knowledgeBase.logLikelihoods.get(feature));
                logprob += occurrences * knowledgeBase.logLikelihoods.get(feature).get(category);
//                if(!disable)
//                if(classes.size() != knowledgeBase.logPriors.size())
//                {
//                    LinkedList<Map.Entry<String,Double>> list = new LinkedList<Map.Entry<String,Double>>(knowledgeBase.logLikelihoods.get(feature).entrySet());
...
//                    {
//                        if(!classes.contains(temp.getKey())){
//                            classes.add(temp.getKey());
//                            HashMap<String,Double> temp2 = new HashMap<>();
//                            temp2.put(temp.getKey(), temp.getValue());
//                            val.put(feature,temp2);
//                            break;
//                        }
//                    }
//                }
            ...
            rankingMap.put(category, logprob / doc.tokens.size());
            if (logprob > maxScore) {
                maxScore = logprob;
                maxScoreCategory = category;
            }
        }
//        System.out.println(text + "   score :  "+ maxScore);
        return maxScoreCategory; //return the category with heighest score
    }

    public String predict(String text, String param) throws IllegalArgumentException {
        ...
        for (Map.Entry<String, Double> entry1 : knowledgeBase.logPriors.entrySet()) {
            ...
            //predictionScores.put(category, logprob); 
             ...
             //   System.out.println("The probability is " + Math.exp(logprob));
            sum = sum + Math.exp(logprob);
            if (logprob > maxScore) {
                maxScore = logprob;
        ...
        if (param.equals("")) {
            ...
            return maxScoreCategory + "," + maxScore;
        }
